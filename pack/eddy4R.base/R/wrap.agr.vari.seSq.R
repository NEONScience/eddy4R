...
#' @description 
#' Function wrapper. Calculate aggregated variance and squared standard error from a finer to a coarser temporal resolution: from minutely to hourly resolution, from hourly to diurnal cycle, and from diurnal cycle to monthly
...
wrap.agr.vari.seSq <- function(
  ...
  if (MethAgr == "hrly"){
    idx <- base::as.vector(base::sapply(base::unique(data$date), function(xx) (xx + c(0:23)/24)))
  ...
  tmp <- list()
  tmp <- sapply(1:length(idx), function(xx) {
    
    if (MethAgr == "hrly") whr <- which(abs(floor(data$hour) - hour[xx]) < 0.1 & abs(data$date - date[xx]) < 0.1)
    
    if (MethAgr == "mdc") whr <- which(abs(data$hour - (xx-1)) < 0.1)
