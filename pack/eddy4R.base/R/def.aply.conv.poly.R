...
#' @param \code{data} Required. A vector or matrix or type numeric, containing the data to be converted
...
def.aply.conv.poly <- function(data,coefPoly=c(0,1)) {
  ...
  rpt <- predict(object=func,newdata = data) # Convert data using polynomial function
