...
#' the chemical species character strings in Unit$Spcs 
...
#'        }
#'      \item{Type}{A named list of the unit type corresponding to entries in Unit$Base$Symb.
...
#'      used to find the eddy4R internal units (listed in Unit$Intl):\cr\cr
...
#'   eddy4R family of functions. List names are unit types selected from Unit$Base$Type.
#'   List values are unit base symbol and prefix character strings selected from 
#'   Unit$Base$Symb and Unit$Prfx, respectively. Only one unit per type is used internally:\cr\cr
#'    \code{Dist = "m"}\cr
#'    \code{Mass = "kg"}\cr
#'    \code{Time = "s"}\cr
#'    \code{Temp = "K"}\cr
