...
#' @seealso \code{\link{def.unit.info}}, \code{\link{IntlUnit$Intl}}, \code{\link{def.unit.conv}}
...
def.unit.intl <- function(unit) {
  ...
  if((base::length(chkType) == 2) && !base::is.na(base::sum(chkType)) && (infoUnit$sufx[base::which(chkType == 1)] == 1) &&
    ...
    unitNum <- base::paste0(eddy4R.base::IntlUnit$Intl[["Num"]],infoUnit$spcs[base::which(chkType == 1)],sep="") # mol portion
    unitDist <- base::paste0(eddy4R.base::IntlUnit$Intl[["Dist"]],"-3",sep="") # dist portion
  ...
  if((base::length(chkType) == 3) && !base::is.na(sum(chkType)) && (infoUnit$sufx[base::which(chkType == 1)] == 1) &&
    ...
    # let's create the output unit string
    unitNum <- base::paste0(eddy4R.base::IntlUnit$Intl[["Num"]],infoUnit$spcs[base::which(chkType == 1)],sep="") # mol portion
    unitDist <- base::paste0(eddy4R.base::IntlUnit$Intl[["Dist"]],"-2",sep="") # dist portion
    unitTime <- base::paste0(eddy4R.base::IntlUnit$Intl[["Time"]],"-1",sep="") # time portion
  ...
  if((base::length(chkType) == 2) && !base::is.na(sum(chkType)) && (base::length(base::which(infoUnit$sufx == 1)) == 1) &&
    ...
    # let's create the output unit string
    unitNum01 <- base::paste0(eddy4R.base::IntlUnit$Intl[["Num"]],infoUnit$sufx[1],infoUnit$spcs[1],sep="") # mol portion 1
    unitNum02 <- base::paste0(eddy4R.base::IntlUnit$Intl[["Num"]],infoUnit$sufx[2],infoUnit$spcs[2],sep="") # mol portion 2
