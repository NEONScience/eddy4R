...
def.unit.conv <- function(
  ...
  for(idxVar in nameVars) {
    ...
    data[[idxVar]] <- eddy4R.base::def.conv.poly(data=data[[idxVar]],coefPoly=coefPoly[[idxVar]], MethGc = MethGc)
      ...
      if(infoUnitFrom$posBase[idxBase] != infoUnitTo$posBase[idxBase]) {
        ...
        if(infoUnitFrom$type[idxBase] != infoUnitTo$type[idxBase]){
          ...
          if((infoUnitFrom$type[idxBase] == "Mass" && infoUnitTo$type[idxBase] == "Num") || 
            ...
            if(base::is.na(infoUnitFrom$posSpcs[idxBase]) || base::is.na(infoUnitTo$posSpcs[idxBase])) {
              ...
              if (infoUnitFrom$type[idxBase] == "Mass") {
                ...
                data[[idxVar]] <- eddy4R.base::def.conv.poly(data=data[[idxVar]],coefPoly=c(0,molmSpcsFrom^(-infoUnitTo$sufx[idxBase])), MethGc = MethGc) # Convert data using polynomial function
                if (MethGc == TRUE) {base::gc(verbose=FALSE)} # Clean up memory
                ...
                # Convert to g
                data[[idxVar]] <- eddy4R.base::def.conv.poly(data=data[[idxVar]],coefPoly=c(0,molmSpcsTo^(infoUnitTo$sufx[idxBase])), MethGc = MethGc) # Convert data using polynomial function
                if (MethGc == TRUE) {base::gc(verbose=FALSE)} # Clean up memory
          ...
          if(infoUnitTo$sufx[idxBase] == 1) {
            data[[idxVar]] <- eddy4R.base::def.conv.poly(data=data[[idxVar]],coefPoly=coefPolyBase^infoUnitTo$sufx[idxBase], MethGc = MethGc) # Convert data using polynomial function
            if (MethGc == TRUE) {base::gc(verbose=FALSE)} # Clean up memory
          } 
          else {
            data[[idxVar]] <- eddy4R.base::def.conv.poly(data=data[[idxVar]],coefPoly=c(0,coefPolyBase[2]^infoUnitTo$sufx[idxBase]), MethGc = MethGc) # Convert data using polynomial function
