...
############################################################
  ...
  #GENERAL CONVERSIONS
    ...
    #latent heat of vaporization (Eq 2.55 Foken 2008) [J kg-1] == [m2 s-2]
      data$Lv <- 2500827 - 2360 * def.aply.conv.poly(data=data$T_air,coefPoly=eddy4R.base::Conv$KelvCels)
...
############################################################
  ...
  #aircraft heading as vector average
    mn$PSI_aircraft <- def.aply.conv.poly(data=def.conv.cart.az(matrix(colMeans(def.conv.az.cart(def.aply.conv.poly(data=data$PSI_aircraft,coefPoly=eddy4R.base::Conv$DegRad)), na.rm=TRUE), ncol=2)),coefPoly=eddy4R.base::Conv$RadDeg)
...
############################################################
  ...
  #rotation angle
    rotang <- (def.aply.conv.poly(data=(mn$PSI_uv+180),coefPoly=eddy4R.base::Conv$DegRad)) %% (2*pi)
