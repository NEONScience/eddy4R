...
############################################################
  ...
  #GENERAL CONVERSIONS
    ...
    #latent heat of vaporization (Eq 2.55 Foken 2008) [J kg-1] == [m2 s-2]
      data$Lv <- 2500827 - 2360 * eddy4R.base::def.conv.unit(data=data$T_air,unitFrom="K",unitTo="C")$dataConv[[1]]
...
############################################################
  ...
  #rotation angle
    rotang <- (eddy4R.base::def.conv.unit(data=(mn$PSI_uv+180),unitFrom="deg",unitTo="rad")$dataConv[[1]]) %% (2*pi)
